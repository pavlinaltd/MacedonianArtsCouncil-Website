				...
				for (
					...
					switch (a) {
						case ':root':
							...
							break;
						case ':upper':
							if (!d.parent().parent().is('.nav-dropdown'))
								return !1;
							break;
						case ':opened':
						case ':closed':
							if ((':opened' == a) != d.parent().hasClass('open'))
								return !1;
						case ':toggle':
							if (!d.is('[data-toggle="dropdown-submenu"]'))
								return !1;
							break;
						default:
			...
			if (a.target) {
				if (this === document)
					if (c(a.target).is('a:not([disabled], .disabled)'))
						...
						var e =
							(a.targetWrapper &&
								c(a.targetWrapper).find('.nav-dropdown')) ||
					...
					((e = c(a.target).next()),
					e.is('.nav-dropdown') && f._isCollapsed(e))
				)
					return;
